...
const Dashboard: React.FC<DashboardProps> = ({ tasks, activities, darkMode }) => {
  ...
  const productivityData = useMemo(() => {
    ...
    return dates.map(date => {
      const count = activities.filter(a => a.timestamp.startsWith(date)).length;
      return {
               ...
               <BarChart data={useMemo(() => {
                  ...
                  for (let i = 5; i >= 0; i--) {
                    ...
                    const created = activities.filter(a => {
                      const ad = new Date(a.timestamp);
                      return ad >= weekStart && ad <= weekEnd && (a.action === 'Created task' || a.action.startsWith('Created scope')); 
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className={`p-4 rounded-xl border transition-colors ${
                ...
                <p className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${darkMode ? 'text-[#8C9BAB]' : 'text-slate-400'}`}>Scope</p>
