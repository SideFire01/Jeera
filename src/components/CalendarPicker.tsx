...
const CalendarPicker: React.FC<CalendarPickerProps> = ({ selectedDate, onSelect, onClose, darkMode }) => {
  ...
  return (
    <div className={`p-4 rounded-xl shadow-2xl border w-64 ${darkMode ? 'bg-[#1D2125] border-[#38414a] text-[#B6C2CF]' : 'bg-white border-slate-200 text-slate-800'}`}>
      ...
      <div className="mt-3 pt-3 border-t flex justify-center border-slate-100 dark:border-white/10">
          <button
             type="button"
             onClick={() => onSelect(undefined)}
             className="text-[10px] font-bold uppercase tracking-wider opacity-60 hover:opacity-100 transition-opacity text-red-500"
          >
             Cancel
          </button>
       </div>
